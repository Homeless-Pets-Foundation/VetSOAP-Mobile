...
8. **Sign-out awaits PHI cleanup before clearing auth.** `handleSignOut` in `AuthProvider.tsx` awaits all cleanup (stash storage, stash audio, cache audio, editor temp, drafts) via `Promise.all` **before** `setUser(null)`+`setSession(null)`. Prevents next-user-signs-in-during-prev-cleanup race on shared tablets. Cleanup has retry + `.catch()`. In-memory state (audio editor bridge, clipboard) also cleared.
...
- **APP_VARIANT:** `app.config.ts` exposes `extra.isProduction` from `APP_VARIANT=production`; gates prod-only features at runtime (e.g. screen capture prevention).
...
- `src/auth/AuthProvider.tsx` — `handleSignOut` awaits stash+drafts PHI cleanup before clearing state. `fetchUser()` calls `setStashUserId()`+`draftStorage.setUserId()`. `registerDevice()` on sign-in + restore, sends `ios_phone`/`ios_tablet`/`android_tablet` via `Platform.isPad` (rule 23). `signIn` retries on `AuthRetryableFetchError` (rule 22). MFA helpers here; `mfaAuthRequest()` follows rule 25.
...
**Sign-out cleanup:** `handleSignOut` → `draftStorage.clearAll()` in `Promise.all` alongside stash cleanup; blocks `setUser(null)`/`setSession(null)` until PHI gone (rule 8).
