...
- `src/api/qualityAnalytics.ts` — clinic quality dashboard fetch (`GET /api/organization/dashboard`), Zod-parsed summaries/breakdowns/processing-latency percentiles.
...
- `src/lib/recordingPermissions.ts` — client role-permission matrix (`canRecordAppointments`, `getRecordingPermissions` — edit/delete/export/copy/retry per role) + support-staff recovery messaging. Mobile analog of Connect's `ROLE_PERMISSIONS` — change one, check the other.
