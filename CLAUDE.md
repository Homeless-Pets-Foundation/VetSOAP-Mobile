...
- `src/api/providerIssues.ts` + `src/components/ProviderIssueBanner.tsx` — surfaces BYOK provider fallback/failure to org admins (`credentialScope organization|platform`, `actionableByOrgAdmin`, outcomes `fallback_success|fallback_failure|primary_failure_no_fallback`).
...
- **Error phase tagging:** upload errors phase-tagged at throw sites via `tagPhase()`/`phaseError()`. `UploadPhase` union lives in `src/api/uploadRetry.ts` (re-exported by `recordings.ts`): `preflight`, `silent_check`, `prepare`, `presign`, `r2_put`, `confirm`, `create_draft`, `unknown`. `uploadSlot` in `record.tsx` reads `getUploadPhase(error)` in catch → routes to all three layers. Do NOT pattern-match error messages — tag at the throw site.
...
- **`src/lib/pendingConfirm.ts`** (+ `PendingConfirm` in `src/types/multiPatient.ts`) — validated post-PUT upload-confirmation proof (UUID + audio-type constrained) letting a draft finish submission even when local audio is gone; handed into native durable start. Core durability/idempotency primitive.
