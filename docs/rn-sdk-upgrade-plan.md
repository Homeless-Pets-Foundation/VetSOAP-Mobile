...
1. Option A on its own branch — take the SDK-55-safe bumps, get CI green.
   - **Two JS-only packages still need runtime checks.** `@supabase/supabase-js` owns `SIGNED_OUT` semantics, refresh, and token rotation — the exact behaviour rules 16, 17 and 22 work around — and `tests/auth-*.test.mjs` assert source patterns against storage doubles, never a real GoTrue session. `@sentry/react-native` is JS *and* native. If either moves, run the auth checks in §6 (cold-start restore, refresh/expiry recovery, sign-out then sign-in) and the crash-reporting check, regardless of what the rest of the bump set touches.
