...
function runPendingDraftSync(userId: string): Promise<void> | null {
  ...
  const job = measurePhase('pending_draft_sync', { user_scoped: true }, async () => {
    try {
      await draftStorage.syncPending(userId, (formData) => recordingsApi.create(formData, { isDraft: true }));
      ...
      NetInfo.fetch()
        .then((state) => {
          ...
          if (state.isConnected && state.isInternetReachable !== false) {
            runPendingDraftSync(userId)?.finally(() => {
