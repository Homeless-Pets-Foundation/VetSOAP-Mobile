...
function makeDestination({ existing = [], failCreateAt = -1, removeResult = true } = {}) {
  const targets = [];
  const createdNames = [];
  return {
    targets,
...
test('enforces a per-part watchdog and leaves no partial file', async () => {
  const source = manifest({ urls: ['https://r2.test/one'], sizes: [3] });
  const destination = makeDestination();
  const error = await rejectionCode(
    download.downloadAudioManifest({
...
test('watchdog aborts a stalled response stream and rolls back bytes already written', async () => {
  const source = manifest({ urls: ['https://r2.test/one'], sizes: [3] });
  const destination = makeDestination();
  const error = await rejectionCode(
    download.downloadAudioManifest({
