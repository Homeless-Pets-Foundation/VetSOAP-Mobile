...
test('every kill-probe call site is cancellation-guarded', () => {
  ...
  const calls = [...src.matchAll(/reportPriorUncleanExitDetached\(/g)];
  ...
  assert.equal(
    (src.match(/if \(!isCancelled\(\)\) reportPriorUncleanExitDetached\(userId, EMPTY_MANIFEST_IDS, isCancelled\);/g) ?? []).length,
    2,
    'both early-return probes must be guarded',
  );
  assert.match(src, /reportPriorUncleanExitDetached\(userId, new Set\(manifests\.map/);
});
