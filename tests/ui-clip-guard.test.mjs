...
const SWEPT = [
  ...
  { file: 'src/components/AttentionFeedSection.tsx', anchor: '{summaryLabel}', mitigation: 'width', multiToken: true,
    why: 'practice-wide count ("2 need you · 3 across the practice")' },
  { file: 'app/(app)/(tabs)/index.tsx', anchor: 'Welcome{user?.fullName', mitigation: 'width', multiToken: true,
    why: 'greeting shares a row with the wordmark and the gear' },
  // ui/Collapsible — one edit, many call sites (Clinic Quality, breakdown rows).
  { file: 'src/components/ui/Collapsible.tsx', anchor: '{title}', mitigation: 'width', multiToken: true },
  { file: 'src/components/ui/Collapsible.tsx', anchor: '{headline}', mitigation: 'width', multiToken: true,
    why: '"500 completed · 29% missing details · 7 min to 90%" — losing the tail drops a metric' },
  { file: 'src/components/ui/Collapsible.tsx', anchor: 'clipSafe(badge)', mitigation: 'headroom', multiToken: true },
